...
; (function () {
    ...
    smart.load = function (instanceID) {
        if (this.isNull(instanceID)) {
            smart.ajaxPost({
                ...
                data: { relation: JSON.stringify(smart.env.config), instanceID: instanceID },
    ...
    smart.start = function (id,instanceID) {
        if ($.isFunction(smart.beforeStart) && $.isFunction(smart.doValidation)) {
            if (smart.doValidation.call(window)) {
            ...
            alert('请订阅beforeStart和doValidation事件');
        }
    }
})();